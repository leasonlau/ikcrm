...
\*\*_**这里的所属部门并不是指在这些部门的用户负责的数据，而是指数据对应的所属部门。比如用户1和用户2，用户1的数据权限是所属部门，用户1在A部门，用户2在A、B部门。用户2新增客户的时候可以选择所属部门为A或B，如果用户2有一个客户1的所属部门是A，那么用户1就可以看到这个客户1。如果用户2有一个客户2的所属部门是B，那么用户1就看不到这个客户2了。**_
